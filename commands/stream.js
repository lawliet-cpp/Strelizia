...
module.exports.run = async(bot,message,args)=>{
    ...
    if(!voice_channel) return message.reply('**You have to be in a voice channel**')
    const permissions = voice_channel.permissionsFor(bot.user)
    if(!permissions.has("SPEAK")) return message.reply("I don't Have the required permissions")
    if(!permissions.has("CONNECT")) return message.reply("I don't Have the required permissions")
