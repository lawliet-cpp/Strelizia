...
module.exports.run = async (bot, message, args) => {
  ...
  if (!permissions.has("CONNECT"))
    return message.channel.send("**I don't have the required Permission**");
  ...
  if (message.content.startsWith("splay")) {
    ...
    if (valid) {
      ...
      song = {
        title: videos_result.videos[0].title,
        url: videos_result.videos[0].url,
      };
    }
    if (!server_queue) {
      queue_constructor = {
...
const play = async (guild, song) => {
  ...
  if (!song) {
    ...
    server_queue.connection.play(stream).on("finish", () => {
